...
RED = (255,0,0)
CYAN = (0,255,255)
BLINK = [(244,255,255),(192,240,255),(128,224,255),(64,192,255),(128,224,255),(192,240,255)]
...
EMY_NAME = [
    "Green slime","Red slime","Axe beast","Sword man","Death hornet","Signal slime","Deveil plant","Twin killer","Hell"
    ]
...
def draw_dungeon(bg,fnt):#ダンジョンを描画する
    ...
    draw_para(bg,fnt)#主人公のパラメータを表示

def put_event():#床にイベントを設置する
    ...
    while True:
        ...
        if(dungeon[y][x] == 0):
            for ry in range(-1,2):#階段の周りを床にする
                for rx in range(-1,2):
                    dungeon[y+ry][x+rx] = 0
            dungeon[y][x] = 3
            break
...
def move_player(key):#主人公の移動
    ...
    if dungeon[pl_y][pl_x] == 1:#宝箱に乗った
        dungeon[pl_y][pl_x]= 0
        treasure = random.choice([0,0,0,1,1,1,1,1,2,2])
        if treasure == 0:
            potion = potion+1
        if treasure == 1:
            blazegem = blazegem+1
    ...
    if dungeon[pl_y][pl_x] == 2:#繭に乗った
        ...
        if r < 20:#食料
            treasure = random.choice([3,3,3,3,4])
    ...
    if dungeon[pl_y][pl_x] == 3:#階段に乗った
        ...
        tmr = 0
        return
    if dungeon[pl_y][pl_x] == 4:#トラップ床に乗った
        dungeon[pl_y][pl_x] = 0
        r = random.randint(0,99)
        if r > 10 and pl_life > 10:
            pl_life = pl_life-10
        if r <= 10 and pl_life > 5:
            pl_life = 5
        return
    #方向キーで上下左右に移動
    x = pl_x
    ...
    if key[K_RIGHT] == 1:
        ...
        if dungeon[pl_y][pl_x+1] != 9:
            pl_x = pl_x+1
    pl_a = pl_d*2
    if pl_x != x or pl_y != y : #移動したら食材の量と体力の計算
...
def draw_para(bg,fnt):#主人公のパラメータを表示
    X = 30
    Y = 600
    bg.blit(imgPara,[X,Y])
    col = WHITE
...
def draw_battle(bg,fnt):
    ...
    draw_bar(bg,340,580,200,20,emy_life,emy_lifemax)
    if emy_blink > 0:
...
def main(): #メイン処理
    ...
    while True:
        ...
        if idx == 0:#タイトル画面
            ...
            draw_text(screen,"Press enter key!",320,560,font,BLINK[tmr%6])
            if key[K_RETURN] == 1:
                make_dungeon()
                put_event()
                floor = 1
            ...
            if tmr == 1:
                ...
                draw_text(screen,emy_name+"Encounter!",350,200,font,WHITE)
            elif tmr <= 16:
            ...
            if tmr == 5:
                if random.randint(0,99) < 60:
                    ...
                    set_message("You failed to free")
            if tmr == 10:
                idx = 13
                tmr = 0
